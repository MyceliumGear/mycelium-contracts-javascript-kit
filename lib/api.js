...
MyceliumMultisig.prototype._requestPayload = function(callback) {
  ...
  this._doPostRequest(url, null, function(err, response) {
...
MyceliumMultisig.prototype.login = function(username, password, callback) {
  ...
  this._doPostRequest(url, opts, function(response) {
    ...
    if (result) {
      self._requestPayload(function(success, result, response) {
        if (success == false) {
            this._onNewPayload(password, function() {
            });
        } else {
            this._onProcessPayload(password, function() {
            });
        }
      });    
    }
